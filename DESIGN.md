...
| Footprint | 420×672 pt, including expanded setup, the `menu bar + hotkey` row and the `bridge` row |
...
inside the same 420×672 shell. The first drawn frame is the assembled character; the
...
timing; its animation is disabled for reduced motion. The 420×672 shell and
