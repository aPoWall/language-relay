...
| M6 | the voxel character stands in the panel header at 40 pt and answers the cursor and a click; the flat mark lives in the bar, About and the favicons | wave 10 § B | done · `product-character` in `--ui-self-test`, the stand shot `relay-testbed.png`; `AXPress` on the character is refused by design, the gesture needs a key window (`docs/QA-2026-09-17-menubar.md`) | 2026-09-17 |
...
| C3 | release through `release-app.mjs relay --install`, zip, SHA, release notes, page version | wave 3 § C, rule 13 | done for 2.5.1 · zip `a33079d6…358e1464` in `sites/apps/language-relay/`, `SHA256SUMS.txt` and the page version updated, nothing committed in `lab-sites` | 2026-09-17 |
